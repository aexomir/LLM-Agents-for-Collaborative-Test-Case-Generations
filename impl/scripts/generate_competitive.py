...
def generate_competitive_tests(
    ...
    if use_timestamp:
        base_dir = output_dir.parent if output_dir.name == "competitive" else output_dir.parent.parent
        actual_output_dir, run_id = create_timestamped_output_dir(base_dir, "competitive")
    ...
    try:
        ...
        print(f"Calling LLM for Agent 1 (provider: {provider})...")
        response = call_llm(
            prompt=prompt,
            provider=provider,
            temperature=0.7,
        ...
        print(f"Agent 1 syntax validation: {message}")
        ...
        if is_valid:
            ...
            print(f"Agent 1 quality score: {quality_score:.1f}%")
    ...
    try:
        ...
        provider = os.getenv("LLM_PROVIDER", "local" if not os.getenv("OPENAI_API_KEY") else "openai")
        print(f"Calling LLM for Agent 2 (provider: {provider})...")
        response = call_llm(
            prompt=prompt,
            provider=provider,
            temperature=0.7,
        ...
        test_code = extract_python_code_from_response(response)
        is_valid, message = validate_test_code(test_code)
        print(f"Agent 2 syntax validation: {message}")
        ...
        if is_valid:
            ...
            quality_score = quality_metrics.get("quality_percentage", 0.0)
            print(f"Agent 2 quality score: {quality_score:.1f}%")
    ...
    if run_id:
        import_path = f"impl.tests_generated.competitive.{run_id}.test_{cut_module}"
    else:
        import_path = f"impl.tests_generated.competitive.test_{cut_module}"
...
def main():
    ...
    try:
        output_dir, run_id = generate_competitive_tests(
